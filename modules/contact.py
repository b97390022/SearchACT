...
class Contact:
    def __init__(self, cwd = None):
        self._cwd = cwd or (os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else pathlib.Path(__file__).parent.parent.resolve())
    ...
    def make_dict(self) -> dict:
        assert self.path_file != None, "檔案不存在，請先設定檔案路徑。 e.g self.path_file = 'some_path_to_file'"
        ...
        for (
            ...
            dict_mapping_data.setdefault(
                {i.name: i.abbreviation for i in get_department_all()}.get(str(department), str(department)), set()
            ).add(key_)
            # tokenized name
    ...
    def check_version(self):
        try:
            ...
            if (
                excel_version != ""
                and self.data.get("*version") != excel_version
            ):  
    ...
    def add_searchTerm_to_key(self, str_input_Term, str_input_KEY):
        ...
        self.save_pickle(
            path.splitext(self.path_file)[0], self.data
        )
