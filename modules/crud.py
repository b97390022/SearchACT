...
from .database import get_engine, get_database_session
...
def create_or_update_departments(db: Session, departments: List[DepartmentSchema]) -> List[DepartmentSchema]:

    departments_dic = {
        department.name: (
            Department(**department.dict(exclude_defaults=True))
        )
    ...
    for department in (
        db.query(Department)
        .filter(Department.name.in_(departments_dic.keys()))
        .all()
    ):  
...
def get_department_all(db: Session = next(get_database_session())) -> List[DepartmentSchema]:
